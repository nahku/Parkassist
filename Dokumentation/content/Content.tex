...
In ASCET mainly graphical programming using block diagrams is used due to requirement D7. D8 is graphically programmed as well because the pulse signal in Simulink is programmed graphically. Exceptions are the unit tests. The test are programmed in ESDL because some test have a lot of test cases which would lead to a large block diagram that would likely be unclear.\\
For the Simulink models the ode8 solver is used, because it is the most precise fixed step solver. As step-width, 0.01s is used to provide efficiency but also the necessary precision.
...
This leaves a margin for velocity measurement inaccuracies, which will be analysed in chapter \ref{cha:D5}f.
...
\section{Inaccuracy in car velocity}\label{sec:D5_v}
...
First, messages are defined to describe the needed data interfaces. Listing \ref{lst:D7_Messages} shows the three defined message interfaces. The first interface represents messages regarding the car including its velocity, its acceleration, its position, the ultrasonic distance and the pulse signal. The second interface contains a message that describes the current brake pedal position. The third interface represents the status of the system test.
...
\section{Unit tests for ParkAssistControllerClass}
...
\begin{table}[H]
\centering
\caption{Equivalence classes for unit testing ParkAssistControllClass}
...
\begin{table}[H]
\centering
\caption{Test cases for unit testing ParkAssistControllerClass}
\begin{adjustbox}{width=0.5\textwidth, center=\textwidth}
\renewcommand{\arraystretch}{1}
\begin{tabular}{llll}
...
\section{Unit tests for FrequencyComputationClass}\label{sec:unitfrequency}
...
\begin{table}[H]
\centering
\caption{Equivalence classes for unit testing FrequencyComputationClass}
\begin{adjustbox}{width=1\textwidth, center=\textwidth}
\renewcommand{\arraystretch}{1}
\begin{tabular}{lllllll}
...
\begin{table}[H]
\centering
\caption{Test cases for unit testing FrequencyComputationClass}
\begin{adjustbox}{width=1\textwidth, center=\textwidth}
\renewcommand{\arraystretch}{1}
\begin{tabular}{llll}
...
\section{Unit tests for PulseSignalClass}
...
\begin{table}[H]
\centering
\caption{Equivalence classes for unit testing PulseSignalClass}
\begin{adjustbox}{width=1\textwidth, center=\textwidth}
\renewcommand{\arraystretch}{1}
\begin{tabular}{lllllll}
...
\begin{table}[H]
\centering
\caption{Test cases for unit testint PulseSignalClass}
\renewcommand{\arraystretch}{1}
\begin{tabular}{lll}
...
The goal of the system test is to see if the system fulfils all given requirements. The system test is using a state machine that can be seen in figure \ref{fig:D10_sm}. The state machine checks requirements R1 and R3. The status of the state machine is set to true if no error occurred. There are three type of failures that lead to an error:

\begin{itemize}
\item The velocity equals zero and the pulse signal is high. If the velocity equals zero, the pulse signal has to be low (R3).
\item The velocity is greater than zero, the position is greater or equal 1.9 m and the pulse signal is low. In this case the pulse signal has be continuous (high) (R3). 
\item The position of the car exceeds 2 m. The car needs to stop within 2 m (R1).
\end{itemize}  
...
The class $StateMachineBD$ that is shown in figure \ref{fig:D10_smbd} is responsible for passing the velocity, the pulse signal and the position to the state machine as well as sending the system test message.

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{images/stateMachineBD.png}
\caption{ASCET block diagram of $StateMachineBD$}
\label{fig:D10_smbd}
\end{figure}

To execute the experiment, a schedule has to be defined. The used schedule is described in chapter \ref{cha:D7} and can be seen in Figure \ref{fig:D7_schedule}. With the parameters that have been evaluated in the previous chapters, the car stops at 1.905 m.
...
The check box at the right bottom shows the results of the state machine.
The graphs plotted by the oscilloscope are evaluated in the following to check the requirements of the system that are not covered by the state machine. 

The upper channel of the oscilloscope shows the velocity that becomes zero once it is below 0.29 km/h after approximately 1.4 s (R5).
The channel below the velocity shows the brake pedal function p(t) that follows a smooth human like behaviour (R2). The middle channel shows the acceleration that is influenced by p(t). The channel below the acceleration shows the position of the car in turquoise and the ultrasonic distance in blue. The ultrasonic distance is implemented based on R4.
%The third subplot shows the covered distance of the car, which is clearly under the 2 m mark. 
The bottom channel shows the pulse signal. The period of the pulse signal is getting shorter once the car approaches the target position. The plot also shows that the pulse signal starts once the car is slower than 3.6 km/h (\^= 1m/s) and ends once the car is standing. The exact stopping position of the car is 1.905 m which is why the pulse signal becomes continuous for a short time and then gets reset, once the car stops (R3). 
...
\caption{Ultrasonic sensor limits 2}
...
Inaccuracy in velocity measurement (Requirement R5) have been discussed in section \ref{sec:D5_v}.
...
This chapter consists of a reflection on the project and a description which observations have been made concerning the planning, the modeling, the requirements, the prescribed functions, our solution, the testing and the selected graphical approach.

\begin{itemize}
\item The Planning was very helpful and it is interesting to compare the expected complexity and the actual complexity of the tasks. Overall, we are satisfied with our estimation of the complexity. The only thing we underestimated significantly was the generation of the pulse signal.
\item The model is not completely realistic. There is 
no possibility to accelerate and a drop from 0.29 km/h to 0 km/h is also unrealistic. The model could also be extended to include objects at variable distances so that it would be necessary to include the ultrasonic distance in the controller.
\item Requirements D5 and D13 overlap and it was difficult for us to separate them. Moreover, we were not sure if in requirement D3 the brake pedal function should already be determined or if only an idea for the function should be described.
\item The prescribed functions/values made the implementation easier. It would have been more complicated if for example stops from various initial velocities or within various distances would have had to be considered. The friction and braking constant are comparatively high so that without accelerating it was not possible to get an entirely smooth velocity drop.
\item The developed brake pedal function in our solution is only dependant on the time, therefore external conditions are not recognised.
If, for example a person would step in front of the car, the car would run over that person, even though the ultrasonic sensor might recognise the person. To prevent that the distance from the ultrasonic sensor would have to be included in the ParkAssist controller. In the future, it would be better to implement a controller with feedback for the applied braking pressure.
\item Unit tests were easy to implement and very helpful. This way, bug that have been included along the development could be detected quickly.  
\item Graphical modeling worked out well for us. The only disadvantage that we experienced was that if-clauses can be complex to implement graphically, in particular if there are various if-clauses in series. Having many if-clauses may also lead to an unclear block diagram. 
\end{itemize}
