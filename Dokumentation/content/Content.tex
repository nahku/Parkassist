...
In ASCET mainly graphical programming using block diagrams is used due to requirement D7. Even though D8 does not demand graphical programming, for consistency reasons D8 is being graphically modelled as well. The unit tests are an exception. The test are programmed in ESDL because some test have a lot of test cases which would lead to a large block diagram that would likely be unclear. 

Simulink: ode8 most precise runge-kutta solver
Fixedstep

%Die oberen Diagramme sind mit den Solver-Einstellung FixedStep nach ode4 – RungeKutta mit einer Schrittweite von 0.001s erzeugt worden. Der Solver ode4 wurde ausgewählt, da dieser das beste Verhältnis von Effizienz und ausreichender Genauigkeit bereitstellt und eine kleine Schrittweite für exakte Werte zu Beginn. Für die untere Reihe hingegen wurde ein FixedStep Verfahren mit automatischem Solver und Schrittweite gewählt, die ein instabiles Verhalten durch eine zu groß gewählte Schrittweite zeigt. Als Alternative zu kleinen Schritten ist auch VariableStep, welches besonders sinnvoll für lange Simulationsdauern ist, da sich die Werte nach t > 1.0s nur geringfügig ändern. Der Nachteil dieser Einstellung ist aber eine Ungenauigkeit im vorderen Bereich bei t < 0.3s.
...
This ensures a smooth braking behaviour with starting braking slowly and also easing of the brake at the end of the manoeuvre, using friction to come to a full stop (like the driving teacher suggested).
...
The integrator that integrates the constant 1 provides the time input to the lookup table.
\begin{figure}[H]
\centering
...
If the car should be able to stop before 2 m with an unknown slope, a controller with feedback would have to be used instead of p(t), to dynamically adjust the brake pedal position depending on the car behaviour.
...
This chapter describes how the Simulink model from chapter \ref{cha:D2} is transferred to ASCET. %The model is also extended  to include a time dependent brake pedal function as described in chapter \ref{cha:D3}. 
...
The acceleration has the form of a parable as desired.
Due to the friction and braking constant the velocity is steeper than the curve that can be seen in chapter \ref{cha:D3}. If one would like to have a similar velocity curve it would be necessary to apply gas in order to brake more slowly which is not possible in this model as there is no gas pedal.
The third subplot shows the covered distance of the car, which is clearly under the 2 m mark. 
...
Depending on the sensor, the sensor frustum might be different, but the concept is the same.
...
The inaccuracy of the ultrasonic sensor of +/- 2 cm do not have an influence on the current p(t).
...
The inaccuracy would have to be considered.
