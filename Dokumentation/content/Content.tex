...
\begin{itemize}
\item The Planning was very helpful and it is interesting to compare the expected complexity and the actual complexity of the tasks. Overall, we are satisfied with our estimation of the complexity. The only thing we underestimated was the generation of the pulse signal.
\item The model is not completely realistic. There is 
no possibility to accelerate and a drop from 0.29 km/h to 0 km/h is also unrealistic. The model could also be extended to include moving objects so that it would be necessary to include the ultrasonic distance in p(t).
...
\item The prescribed functions/values made the implementation easier. It would have been more complicated if for example stops from various velocities or within various distances would have been considered. The friction and braking constant are comparatively high so that without accelerating it was not possible to get a smooth velocity drop.
...
If, for example a person would step in front of the car, the car would run over that person, even though the ultrasonic sensor might recognise the person. To prevent that the distance from the ultrasonic sensor would have to be included in the ParkAssist controller. In the future, it would be better to implement a controller for the applied braking pressure instead of a regulation.
\item Unit tests were easy to implement and very helpful. This way, failures that have been made along the way could be detected quickly.  
...
%The model is not completely realistic. There is 
%-no possibility to accelerate
%-drop from 0.29 km/h
%\section{Computation inaccuracies}
%-rundungsfehler
%
%\section{ParkAssist limitations}
%-no steering
%-only 2 meter stop considered
%The developed controller is only dependant on the time, therefore external conditions are not recognised.
%If, for example a person would step in front of the car, the car would run over that person, even though the ultrasonic sensor might recognise the person. To prevent that the distance from the ultrasonic sensor would have to be included in the ParkAssist controller.
%
%-no feedback control