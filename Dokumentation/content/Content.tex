...
The aim of the feasibility study is to analyse whether the introduced model in section \ref{cha:Introduction} based on the given formulas todo. 
...
- included Switch -> wenn Geschwindigkeit kleiner 0,29 folgt daraus Geschwindigkeit = 0 \\
-otherwise car would reverse
...
-result with constant 5\% brake pressure
-stops before 2m
-constant brake pressure not human like but shows that it is possible to stop before 2 m
-brake function can be changed to human like behaviour
...
Off & {0, 1, 1.5, 1.9, 2} & V = 0m/s oder V > 1m/s\\
...
\section{Unit tests ParkAssistControll}
